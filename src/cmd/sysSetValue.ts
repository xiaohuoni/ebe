...
export function sysSetValue({ value, platform, scope, config}: CMDGeneratorPrames): string {
 ...
 const callback = Array.isArray(callback1) && callback1.length ?  `\n${CMDGeneratorFunction(callback1,
  {},
  platform,
  scope,
  config,
 ...
 if (typeof tempCompId === 'string' && temValue?.[tempCompId]) {
  compId = [tempCompId];
  return `// 设置控件的值 \n setValue('${compId}', ${parse2Var(temValue?.[tempCompId])})${callback} ` ;
  ...
  return `// 批量设置控件的值 \n setValue(${parse2Var(valueList)})${callback} `
 } else {
  return '';
 } ;
}
