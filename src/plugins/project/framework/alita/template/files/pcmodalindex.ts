...
export default function getFile(): [string[], ResultFile] {
  const file = createResultFile(
    ...
    'tsx',
    `import React, {
      useState, useRef, useMemo, useImperativeHandle,
    } from 'react';
    import { message } from '@lingxiteam/engine-pc/es/components/EnhanceAntdComp';
    import Modal from './Modal';
    // import { Drawer } from './DynamicDrawer';
    import { LocaleFunction, ModalManagerHooks, OpenDrawerProps, OpenModalProps } from '@lingxiteam/types';
     ...
     * @module ModalManager
     */
    const ModalManager = React.forwardRef<ModalManagerHooks, ModalManagerProps>((props, ref) => {
      // relPageId为后面错误定位所需
      ...
      const open = ({
        type = 'Modal', pageId, onOk, onCancel, params, lcdpParentRenderId,
      }: any) => {
        const instData = type === 'Modal' ? ModalData : DrawerData;
        const instMethod = type === 'Modal' ? setModalData : setDrawerData;
        if (!pageId) {
          message.error(getLocale('Modal.missParams', '弹窗操作失败：缺少页面实例ID或页面路径'));
          return;
        }
      ...
      const _close = (modalInstId: any, pageId?: any, type = 'Modal') => {
        ...
        if (modalInstId) {
          const targetIndex = instData.findIndex((i) => i.modalInstId === modalInstId);
          if (targetIndex !== -1) {
            instData[targetIndex].visible = false;
            instMethod([...instData]);
          }
        } else if (pageId) {
          instData.filter((i) => i.pageId === pageId).forEach((i) => {
            i.visible = false;
          });
          instMethod([...instData]);
        }
      ...
      // 更改弹窗属性值
      const setStatue = ({ modalInstId, pageId, type = 'model', propsName, propsValue }: {modalInstId?: string, pageId?: string, type: 'model' | 'drawer', propsName: string, propsValue: any}) => {
        const instData = type === 'model' ? refModalData.current : refDrawerData.current;
        const instMethod = type === 'model' ? setModalData : setDrawerData;
        if (modalInstId) {
          const targetIndex = instData.findIndex((i: any) => i.modalInstId === modalInstId);
          if (targetIndex !== -1) {
            instData[targetIndex][propsName] = propsValue;
            instMethod([...instData]);
          }
        } else if (pageId) {
          instData.filter((i: any) => i.pageId === pageId).forEach((i: any) => {
      ...
      useImperativeHandle<any, ModalManagerHooks>(ref, () => ({
         ...
         */
        openModal: (pageOptions: OpenModalProps) => open({
          ...pageOptions,
          type: 'Modal',
        }),
         ...
         * @return {number} 此次生成的实例ID
         */
        openDrawer: (pageOptions: OpenDrawerProps) => open({
          ...pageOptions,
          type: 'Drawer',
        }),
