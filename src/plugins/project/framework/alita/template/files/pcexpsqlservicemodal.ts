...
export default function getFile(): [string[], ResultFile] {
  const file = createResultFile(
    ...
    'tsx',
    `import React, {
    useState, useImperativeHandle, forwardRef, useMemo,
  } from 'react';
  import {
  ...
  const ExpSQLServiceModal = forwardRef<ExpSQLServiceModalHooks, ExpSQLServiceModalProps>((props, ref) => {
    const {
      // appId,
      api,
      utils,
    ...
    const selectedRows = useMemo(() => selectedKeys.length ?
      curDataSource.filter((item) => selectedKeys.includes(item.code)) : [], [selectedKeys, curDataSource, params]);
  
    const querySQLServiceRowColumns = async (serviceCode: string) => {
      try {
        setLoading(true);
        const resultList: any[] = await api.getSqlQueryColumns({ 
          serviceCode,
          // appId
        }) as any[] || [];
    ...
    const exportInsts = async ({ sqlServiceCode, sqlServiceFields, attrs, custFileName, exportType, tempParams, onSuccess, onFail, async }: any) => {
      let hide = () => { };
      try {
        ...
        let params = {
          ...
          pickColumns: exportType === 'temp' ? resultList : resultList.filter(r => sqlServiceFields.includes(r.code)),
          params: attrs,
        };
        if (exportType === 'temp') {
          params = {
    ...
    const openWithCustFields = (options: ExpSQLServiceModalOptions) => {
      setParams(options || {});
      const { sqlServiceCode, sqlServiceFields, attrs, custFileName, exportType = 'field', onSuccess, onFail } = options;
      if (!sqlServiceFields?.length && exportType === 'field') {
        setVisible(true);
        querySQLServiceRowColumns(sqlServiceCode);
      } else {
        exportInsts({ ...options, sqlServiceCode, sqlServiceFields, attrs, custFileName, onSuccess, onFail });
      }
    };
