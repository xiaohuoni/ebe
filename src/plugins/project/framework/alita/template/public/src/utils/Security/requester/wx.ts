...
const wxRequest = (
  ...
  if (
    ...
    opts.header?.disabledSignKey === true ||
    opts.header?.disabledEncrypted === true ||
    finallyMode === false
  ) {
    ...
    if (checkIsSignWithSalt(finallyMode)) {
      const saltValue =
        typeof config.sign?.saltValue === 'function'
          ...
          : config.sign?.saltValue;
      opts.header.Cookie = opts.header.Cookie
        ? `${opts.header.Cookie};${config.sign?.saltKey}=${saltValue}`
        : `${config.sign?.saltKey}=${saltValue}`;
    }
