import React, {
  useState, useImperativeHandle, forwardRef, useMemo, useRef,
} from 'react';
import {
  Spin, Divider, Checkbox, message, Modal
} from 'antd';
import engineApi from '@/services/api/engine';
import { openProgressMsg, closeProgressMsg, showProgressNotification, closeProgressNotification } from '../ProgressComp';
// import getApis from '../../services';
import './index';
import { prefix } from '../../styles';
import { ExpBusiObjModalHooks, ExpBusiObjModalOptions, ExpBusiObjModalCustomOptions, BaseModalProps } from '@/types';
...
const ExpBusiObjModal = forwardRef<ExpBusiObjModalHooks, ExpBusiObjModalProps>((props, ref) => {
  const {
    // api,
    // utils,
    // pageId,
    ...
    if (isCustom) {
      ...
      if (Array.isArray(busiObjectFields) && busiObjectFields?.length) {
        return busiObjectFields.map(attrCode => {
          return curDataSource.filter(item => item.attrCode === attrCode)?.[0];
        });
      }
      // return busiObjectFields?.length ? curDataSource.filter((item) => busiObjectFields.includes(item.attrCode)) : [];
    }
    return selectedKeys.length ? curDataSource.filter((item) => selectedKeys.includes(item.attrCode)) : [];
  ...
  const queryBusiObjectRowColumns = async ({ busiObjectId }: any) => {
    try {
      setLoading(true);
      const resultList = await engineApi.queryBusiObjectRowColumns({ busiObjectId }) as any[] || [];
      setCurDataSource(resultList);
      setObjOptions(resultList.map((item) => ({
        value: item.attrCode,
        label: item.attrName,
  ...
  const queryNExportInsts = async ({ busiObjectId, attrs, custFileName, exportType, tempParams, onSuccess, onFail, async }: any) => {
    let hide = () => { };
    try {
      hide = message.loading(getLocale!('export.process.handling', '正在处理中，请稍后...'), 0);
      const resultList = await engineApi.queryBusiObjectRowColumns({ busiObjectId }) as any[] || [];
      setCurDataSource(resultList);
      const fileName = custFileName ? `${custFileName}.xlsx` : `${busiObjectId}-${new Date().getTime()}.xlsx`;
  ...
  const openWithCustFields = (options: ExpBusiObjModalCustomOptions) => {
    ...
    queryNExportInsts({ ...options, busiObjectId, attrs, custFileName, onSuccess, onFail });
  };
  ...
  return (
    ...
    >
      <Spin spinning={loading}>
        <Checkbox
          indeterminate={selectedKeys.length > 0 && selectedKeys.length < objOptions.length}
