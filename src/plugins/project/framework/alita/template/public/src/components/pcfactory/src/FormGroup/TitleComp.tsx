...
type TitleProps = Pick<FormGroupProps, 'hasIcon' | 'iconType'| 'prefixIcon'| 'title'>
type TitleCompProps = {
  prefixCls: string;
  isED?: boolean;
  getEngineApis?: EngineBaseProps['getEngineApis'];
  getEdEngineApi?: IEdCompBaseProps['getEdEngineApi'];
  $$componentItem?: EngineBaseProps['$$componentItem'] | IEdCompBaseProps['$$componentItem'];
} & TitleProps;
const TitleComp = (props: TitleCompProps) => {
  const { iconType, hasIcon, title, prefixCls, prefixIcon = {}, isED, getEdEngineApi, getEngineApis, $$componentItem } = props || {};
  const titlePrefix = `${prefixCls}-title`;
  const renderTitle = () => {
    if (hasIcon || iconType) {
      switch (iconType) {
        case 'line':
          return <div className={`${titlePrefix} ${titlePrefix}-icon-line`}>{title}</div>;
        case 'auto': {
          const {
        ...
        case 'circle':
          return <div className={`${titlePrefix} ${titlePrefix}-circle`}>{title}</div>;
        default:
