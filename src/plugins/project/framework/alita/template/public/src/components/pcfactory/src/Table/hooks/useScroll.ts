...
const useScroll = (props: any) => {
  ...
  const realScroll = useMemo(() => {
    if (tableScroll && typeof tableScroll === 'object' && !Array.isArray(tableScroll)) {
  ...
  const computedTablePosition = () => {
    if (tableWrapRef.current && !tableScroll?.y && adjustModel === 'fill') {
      // 当前表格所在位置
      const tableWrapPosition = tableWrapRef?.current?.getClientRects?.()?.[0] || {};
      // 表格位置超过屏幕 1/2不自适应
      if (tableWrapPosition.y <= window.innerHeight / 2) {
        ...
        const paginationRef = tableWrapRef?.current?.querySelector?.('.pcfactory-pagination');
        ...
        // 表格区域设置固定高度 填充剩余页面
        tableWrapRef.current.querySelector('.pcfactory-table').style.height = `${tableContainerHeight}px`;
  ...
  if (tableWrapRef.current && !tableScroll?.y && adjustModel === 'fill' && tableScrollY > 0) {
