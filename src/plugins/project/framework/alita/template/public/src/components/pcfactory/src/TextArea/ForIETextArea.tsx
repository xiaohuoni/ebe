...
const ForIETextArea = (props: any) => {
  ...
  // 改用原生的事件代替
  const { value, className, onChange, onInput, allowClear, ...restProps } = props;
  const tRef = React.useRef<any>();
  const subRef = React.useRef<any>({ now_value: '' });
  const [compValue, setCompValue] = useState<any>();

  const dSet = React.useCallback(debounce((_value) => {
    setCompValue(_value);
  }, 100), []);
  ...
  return <textarea resize="vertical" className={`${antPrefix}-input ${className}`} value={compValue} ref={tRef} {...restProps} />;
};
