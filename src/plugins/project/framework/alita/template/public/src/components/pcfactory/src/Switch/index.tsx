...
const Switch = LingxiForwardRef<any, SwitchProps>((props, ref) => {
  ...
  const { disabled, required, formFieldsRef, finalRules } = useCommonImperativeHandle(ref, props, {
  ...
  // 渲染开关开启/关闭文案或图标
  const renderSwitchStyle = useCallback((type: string) => {
    if (switchStyle) {
      let Comp = null;
      if (switchStyle === 'font') {
        ...
        if (handleIsValidIconCfg(iconInfo)) {
          Comp = <CommIcon icon={iconInfo} getEngineApis={getEngineApis} $$componentItem={props?.$$componentItem} className="" />;
        }
      }
      if (Comp) {
  ...
  const checkedChildrenComp = useMemo(() => renderSwitchStyle('checkedChildren'), [renderSwitchStyle]);
  const unCheckedChildrenComp = useMemo(() => renderSwitchStyle('unCheckedChildren'), [renderSwitchStyle]);
  ...
  return (
    ...
    >
      <AntdSwitch
        ...
        checked={typeof value === 'string' ? (value.toLowerCase() === 'false' ? false : !!value) : !!value}
        onChange={(e: any) => {
          if (onChange) {
            onChange(e);
          }
