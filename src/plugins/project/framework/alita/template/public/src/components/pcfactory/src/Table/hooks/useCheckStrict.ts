...
function useCheckStrict<T extends { children?: T[]; [key: string]: unknown }>(
  params: CheckStrictlyParams<T>,
) {
  const {
    rowSelectType,
    checkLoosed = false,
  ...
  // 父子数据是否关联选择，默认不关联
  const checkStrictly = useMemo(
    () => !(rowSelectType === 'checkbox' && checkLoosed),
    [checkLoosed, rowSelectType],
  );

  // 标记节点是否禁用
  const { relatedDisabledMap, markDisable, getLevelDataMap, getRecordByKey } =
    useCreation(() => {
        ...
        recursionDataSource<T, LevelItem<T>>(
          ...
          (node, parentPath) => {
            ...
            const n = {
              ...
              __parent__:
                level && parentPath ? parentPath[level - 1] : undefined,
  ...
  const triggerSingleSelection = (originSelectRow: T[], selectRow: T) => {
    ...
    const checked =
      originSelectRow.findIndex((c) => c[currentRowKey] === key) === -1;
    ...
    const checkedRows = [...checkedSet].map((k) =>
      getRecordByKey(k, selectionList),
    );
