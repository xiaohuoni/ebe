...
const WrapperInput = React.forwardRef<any, WrapperInputProps>((props, ref) => {
  const { trigger = 'onChange', valuePropName = 'value', formFieldValueChange, clearValue } = props;
  ...
  const formatArgs = (...args: any) => {
    try {
      ...
      if (_args.length > 0) {
        ...
        } else if (_args[0].target?.hasOwnProperty(valuePropName) && _args[0].target?.[valuePropName] === undefined) {
  ...
  return React.cloneElement(props.children as any, {
    ...
    // @ts-ignore
    className: `${props.wrapperClassName} ${props.children.props.className || ''}`,
    disabled: props.disabled,
    [trigger]: (...args: any) => {
...
export interface FormFieldsHooks {
  changeValidateStatus(validateStatus: FormItemProps['validateStatus'], help?: string): void;
...
export const FormFields = React.forwardRef<FormFieldsHooks, FormFieldsProps>((props, ref) => {
  ...
  useEffect(() => {
    if (!inForm || !uid || !fieldName || typeof formContext.uploadChildRelation !== 'function') {
      return;
    }
  ...
  const childProps = useMemo<{
    ...
    if (inForm) {
      ...
      return {
        ...
        wrapperStyle: wrapperCol !== 'auto' ? { width: wrapperCol } : { flex: 1 },
      };
    }
    return {} as any;
  }, []);
  ...
  useUpdateEffect(() => {
    ...
    const finalValue = props.handleFormValue ? props.handleFormValue(props.value) : props.value;
    // 控件在表单内需要更新值到form中
