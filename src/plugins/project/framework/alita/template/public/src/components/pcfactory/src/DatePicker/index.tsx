...
const parseDateFromFormat = (
  ...
  const date = moment(val, format, true)?.isValid()
    ? moment(val, format)
    : moment(val);
...
const WrapperDatePicker: React.FC<WrapperDatePickerProps> = ({ children }) =>
  React.cloneElement(children, {
    className: `${DATEPICKER_WRAPPER_CLASSNAME} ${
      children.props.className || ''
    }`,
  });
const { RangePicker } = AntdDatePicker;
const FormDatePicker: React.FC<FormDatePickerProps> = forwardRef(
  (props, ref) => {
    const {
      value,
      onChange,
    ...
    useEffect(() => {
      ...
      if (pickerType === 'RangePicker') {
        ...
        v = Array.isArray(value)
          ? value.map((c) => parseDateFromFormat(c, [format, valueFormat]))
          : undefined;
        // 时间控件初始化取值顺序  value--> defaultValue
        if (Array.isArray(value)) {
          v = value.map((c) => parseDateFromFormat(c, [format, valueFormat]));
        } else {
    ...
    const CompPicker =
      pickerType === 'RangePicker' ? RangePicker : (AntdDatePicker as any);
    ...
    const [mode, thisShowTime, picker] = useMemo(() => {
      ...
      if (timeMode) {
        ...
        _mode =
          pickerType === 'RangePicker' ? [_timeMode, _timeMode] : _timeMode;
        _picker = _timeMode;
      }
      // 为不影响时间控件使用，当mode为date时，即默认值时，不特意添加mode属性
      return [
        _timeMode === 'date' ? {} : { mode: _mode },
        _showTime,
        ['year', 'month'].includes(_picker) ? { picker: _picker } : {},
      ];
    ...
    return (
      <CompPicker
        ...
        format={format}
        placeholder={
          pickerType === 'RangePicker' ? undefined : placeholder ?? ''
        }
        value={time}
        open={open}
...
const DatePicker = LingxiForwardRef<any, DatePickerProps>((props, ref) => {
  ...
  const defaultValue = useMemo(() => {
    ...
    return props.defaultValue
      ? parseDateFromFormat(props.defaultValue, [
          finalValueFormat,
          _format,
        ])?.format(finalValueFormat)
      : undefined;
    ...
    if (_val !== undefined) {
      ...
      if (pickerType === 'RangePicker') {
        formattedval = Array.isArray(_val)
          ? // @ts-ignore
            _val.map((c) =>
              parseDateFromFormat(c, [finalValueFormat, _format])?.format(
                finalValueFormat,
              ),
            )
          : undefined;
        ...
        // @ts-ignore
        formattedval = parseDateFromFormat(_val, [
          finalValueFormat,
          _format,
        ])?.format(finalValueFormat); // eslint-disable-line no-underscore-dangle
      }
      ...
      switch (limitRange) {
        case 'afterToday':
          if (moment(val) < moment(new Date().setHours(0, 0, 0, 0))) {
            if (callback) {
              callback(
                customTip || getLocale('DatePicker.afterToday', { name }),
              );
            }
        ...
        case 'period':
          if (startTime && endTime && moment(startTime) <= moment(endTime)) {
            if (
              moment(val) < moment(startTime) ||
              moment(val) > moment(endTime)
            ) {
              if (callback) {
                callback(
                  customTip ||
                    getLocale('DatePicker.period', {
                      startTime,
                      endTime,
                      name,
                    }),
          ...
          if (startTime && moment(val) < moment(startTime)) {
            if (callback) {
              callback(
                customTip || getLocale('DatePicker.after', { name, startTime }),
              );
            }
            return Promise.reject(
              new Error(
          ...
          if (endTime && moment(val) > moment(endTime)) {
            if (callback) {
              callback(
                customTip || getLocale('DatePicker.before', { name, endTime }),
              );
            }
            return Promise.reject(
              new Error(
      ...
      render={(val) => {
        if (val !== undefined) {
          return renderDatePicker(
            Array.isArray(val)
              ? val.map((v) =>
                  parseDateFromFormat(v, [finalValueFormat, _format])?.format(
                    _format,
                  ),
                )
              : (parseDateFromFormat(val, [finalValueFormat, _format])?.format(
                  _format,
                ) as any),
            props.separator,
          );
    ...
    >
      <FormDatePicker
        disabledDate={(current) => {
          ...
          const startTimeDate = parseDateFromFormat(
            myStart,
            parseFormats,
          )?.format(_format);
          const endTimeDate = parseDateFromFormat(myEnd, parseFormats)?.format(
            _format,
          );
          return (
            moment(currentDate) < moment(startTimeDate) ||
            moment(currentDate) > moment(endTimeDate)
          );
        }}
        disabledTime={(current) => {
          ...
          const startTimeDay = moment(myStart, parseFormats, true)?.format(
            _formaDay,
          );
          const endTimeDay = moment(myEnd, parseFormats, true)?.format(
            _formaDay,
          );
          ...
          if (startTimeDay == currentTimeDay) {
            ...
            if (currentHour > startHour) {
              return {
                disabledHours: () =>
                  startHour ? [...Array(startHour).keys()] : [], // 如果当前日期和startTime相同，禁止选择startTime之前的小时
                disabledMinutes: () => [],
                disabledSeconds: () => [],
              };
              ...
              if (currentMinute > startMinute) {
                return {
                  disabledHours: () =>
                    startHour ? [...Array(startHour).keys()] : [], // 如果当前日期和startTime相同，禁止选择startTime之前的小时
                  disabledMinutes: () =>
                    startMinute ? [...Array(startMinute).keys()] : [], // 如果当前日期和startTime相同，禁止选择startTime之前的分钟
                  disabledSeconds: () => [],
                };
              }
            }
            ...
            return {
              ...
              disabledMinutes: () =>
                startMinute ? [...Array(startMinute).keys()] : [], // 如果当前日期和startTime相同，禁止选择startTime之前的分钟
              disabledSeconds: () =>
                startSecond ? [...Array(startSecond).keys()] : [], // 如果当前日期和startTime相同，禁止选择startTime之前的秒钟
            };
          }
          // eslint-disable-next-line eqeqeq
          if (endTimeDay == currentTimeDay) {
            ...
            if (currentHour < endHour) {
              return {
                disabledHours: () =>
                  endHour
                    ? [...Array(24 - endHour - 1).keys()].map(
                        (h) => h + endHour + 1,
                      )
                    : [], // 如果当前日期和endTime相同，禁止选择endTime之后的小时
                disabledMinutes: () => [],
                disabledSeconds: () => [],
              };
            ...
            if (currentHour === endHour) {
              if (currentMinute < endMinute) {
                return {
                  ...
                  disabledMinutes: () =>
                    endMinute
                      ? [...Array(60 - endMinute - 1).keys()].map(
                          (m) => m + endMinute + 1,
                        )
                      : [], // 如果当前日期和endTime相同，禁止选择endTime之后的分钟
                  disabledSeconds: () => [],
                };
              }
            }
            return {
              ...
              disabledSeconds: () =>
                endSecond
                  ? [...Array(60 - endSecond - 1).keys()].map(
                      (s) => s + endSecond + 1,
                    )
                  : [], // 如果当前日期和endTime相同，禁止选择endTime之后的秒钟
            };
          }
          return undefined;
        }}
