...
export function encryptedStr(str: string, type: string): string {
  if (type === MODE.AES) {
    return AESEncrypt(str, lxDecrypt(config.aes?.key as string), config.aes?.encrypt);
  } if (type === MODE.RSA) {
    return RSAEncrypt(str, lxDecrypt(config.rsa?.publicKey as string), config.rsa?.encrypt) || '';
  } if (type === MODE.DES) {
...
export function decryptedStr(str: string, type: string, config: any = {}): string {
  if (type === MODE.AES) {
    return AESDecrypt(str, lxDecrypt(config.aes?.key as string), config.aes?.decrypt);
  } if (type === MODE.RSA) {
    return RSADecrypt(str, lxDecrypt(config.rsa?.privKey as string), config.rsa?.decrypt) || '';
  } if (type === MODE.DES) {
    message.warn('DES encryption is no longer supported');
  }
  return str;
...
interface optsType { body?: any, headers?: any }
type returnType = [string, { body?: any, headers?: any }];

// 统一请求参数加密
export const encryptedRequestParams = (url: string, opts: optsType, type: string) : returnType => {
  const isFormData = opts.body instanceof FormData;
  
  // search 部分加密
  const encryptedUrl = url.indexOf('?') !== -1 ? url.replace(/[^?]+$/g, (search: string) => {
    const params = search.split('&').map(param => {
      const [k, v] = param.split('=');
      return `${encodeURIComponent(k)}=${encodeURIComponent(encryptedStr(v, type))}`;
    });
    return params.join('&');
  }) : url;
