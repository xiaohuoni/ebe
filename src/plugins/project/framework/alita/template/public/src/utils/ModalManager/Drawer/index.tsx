...
import { useSetState } from '../../hooks';
  ...
  const DM = forwardRef((props: any, ref) => {
    const {
    pagePath, modalInstId, setStatue, managerRef, params, onCancel, className = '', lcdpParentRenderId, getLocale, ...restProp
    } = props;
      ...
      if (mProps.footer !== null || mProps.footer === 1) {
        return (
          <div className="ant-drawer-footer-button">
            <Button onClick={handleClose}>{mProps.cancelText || getLocale('cancelText', '取消')}</Button>
            <Button onClick={handleOk} type="primary">{mProps.okText || getLocale('confirm', '确定')}</Button>
          </div>
        );
      }
