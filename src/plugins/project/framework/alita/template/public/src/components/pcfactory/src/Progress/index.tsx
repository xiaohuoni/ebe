...
const MyProgress = LingxiForwardRef<any, MyProgressProps>((props, ref) => {
  const {
    visible = true, onValueRelease,
    title, style, placement, type, showTitle, color, fontSize, lineHeight, fontWeight,
    showInfo, valueColor, valueFontSize, valueLineHeight, valueFontWeight,
    ...restProps
  } = props;
  ...
  const renderValue = useMemo(() => (
    (showInfo) ? (
      <div
        ...
        style={{
          color: valueColor,
          fontSize: type === 'line' ? (restStyle?.fontSize || valueFontSize) : valueFontSize,
  ...
  ), [showInfo, compValue, type, valueColor, valueLineHeight, valueFontWeight, valueFontSize, restStyle?.fontSize]);
    ...
    <div
      className={
        classNames({
          ...
          [`${prefixCls}-wrapper-vertical`]: placement === 'vertical' && type === 'line',
        })
      }
