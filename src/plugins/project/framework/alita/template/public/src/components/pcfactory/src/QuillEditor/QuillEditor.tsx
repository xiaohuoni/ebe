...
const BaseQuillEditor = React.forwardRef((props: BaseQuillEditorPropsType, ref) => {
  const { value, disabled, className, imageType, heightType, minHeight, maxHeight, height, engineApis, onChange } = props;

  const inputting = useRef<boolean>(false);

  const heightStyle = useMemo(() => (heightType === 'auto' ? { minHeight, maxHeight } : { height }), [heightType, height, maxHeight, minHeight]);
  ...
  const convertBase64UrlToBlob = (urlData: string) => {
    ...
    return new Blob([ia], { type: imgData.split(',')[0].split(':')[1].split(';')[0] });
  };
        ...
        if (imageType !== 'base64') {
          ...
          if (blobV) {
            ...
            if (res?.fileId) {
              ...
              newValue = v.replace(/<img src="data:.*?(?:>|\/>)/gi, `<img src="${link}" />`);
            }
          }
        }
        if (!blobV && onChange) {
...
const QuillEditor = LingxiForwardRef<any, MyQuillEditorProps>((props, ref) => {
  const { height, onChange, className, imageType, heightType = 'fixed', maxHeight, minHeight, getEngineApis, value, disabled } = props;
