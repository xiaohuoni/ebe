...
const useFilter = (props: any) => {
  ...
  const collectFilterDropDownDom = (colParams: { dataIndex: string }) => {
    ...
    innerDataSource.forEach((d: any) => {
      ...
      let td: any = tableDom?.querySelector(`tr[data-row-key='${d[rowKey]}'] > td[td-dataIndex='${dataIndex}']`);
  ...
  const getCurrentFilterList = (dataIndex: string) => {
    ...
    if (valKeys?.length) {
      valKeys.forEach((k: string) => {
        const rowKeys = currentFilter[k]?.rowKeyList.filter((rowKey: string) => {
          return Object.keys(filters || {}).every((dataKey: string) => {
            if (dataKey !== dataIndex) {
              const v = filters[dataKey];
              if (v?.length) {
                const rowVal = tempFilterDomList.current[dataKey]?.originDataMap?.[rowKey];
  ...
  // 筛选自定义下拉菜单
  const renderFilterDropDown = (filterParams: FilterDropdownProps, dataIndex: string, columns: any[]) => {
  ...
  const filterDataSource = useMemo(() => {
    if (Object.keys(filters)?.length) {
      return innerDataSource.filter((record: any) => !checkIfHiddenRow(record[currentRowKey]));
    }
