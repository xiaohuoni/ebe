...
import { SideBarTabsProps } from './PropsType';
...
const TabPage: React.FC<TabPageProps> = (props) => {
  ...
  useEffect(() => {
    if (reloadPageKey) {
      const realKeyArray = reloadPageKey.split('--##sideBarKey##--');
  ...
  return load ? (
    <PageView
      ...
      compId=""
      className=""
      pageViewCompState={{ sideBarItem: item, sideBarIndex: index }}
    />
  ) : null;
};
