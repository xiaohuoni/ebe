  ...
  router(url: string, currentAppPages: Array<any>, appId: string, lcdpParentRenderId: string, state? : Record<string, any>) {
    ...
    if (/^((http|https):)?\/\/[^\s]+/.test(url) || !this.refs.history || !this.data.pages) {
      ...
      if (currentAppPages?.length) {
        pageData = isPageId ? currentAppPages.find((n: any) => n.pageId === url) : currentAppPages.find((n: any) => url.split('?').includes(n.pagePath));
      }
      // this.data.pages 包含所有应用页面
      if (!pageData) {
        pageData = isPageId ? this.data.pages.find((n: any) => n.pageId === url) : this.data.pages.find((n: any) => url.split('?').includes(n.pagePath));
      }
      if (pageData) {
        const [pathname, search] = url.split('?');
        const searchPamams = search ? search.split('&').reduce((pre: any, cur: any) => {
          const [k, v] = cur.split('=');
          if (v) {
        ...
        switch (pageData.pageContainerType) {
          ...
          default: {
            message?.warning(this.getLocale('lcdpApi.router.notSupport', `不支持的页面类型: ${pageData.pageContainerType}`, { type: pageData.pageContainerType }));
            break;
          }
        }
      } else {
