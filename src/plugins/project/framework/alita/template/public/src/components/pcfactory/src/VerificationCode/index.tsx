 ...
 */
        ...
        <div className="normalArea">
          ...
          {displayStyle === 'countdownStyle' && (
            <div
              ...
              onClick={onClick}
            >
              {`${countDownTextMap[countdownText]}${countdownText === 'refresh' ? ` ${readingTime}s` : ''}`}
            </div>
          )}
          {renderPicVerifyCode()}
        </div>
        {errorInfo && <span className={`${prefixCls}-errorNotice`}>{errorInfo}</span>}
      </div>
    );
  };
  ...
  return (
    <FormFields
      ...
      rules={finalRules}
    >
      <InnerView renderView={renderView} onChange={onMyChange} onBlur={onMyBlur} />
    </FormFields>
  );
});
