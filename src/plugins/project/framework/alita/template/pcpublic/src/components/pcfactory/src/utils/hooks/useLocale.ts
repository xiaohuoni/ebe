...
const langOptions = {
  ...
  'checkboxGroup.minRule': (params: any) =>
    `低于最小选项限制[最少选${params?.min}个]`,
  'checkboxGroup.maxRule': (params: any) =>
    `超过最大选项限制[最多选${params?.max}个]`,
  'Icon.imgFormatWarning': '图片格式错误',
  'Input.lenRule': (params: any) =>
    `内容长度须大于等于${params?.minLength}且小于等于${params?.maxLength}`,
  ...
  'Upload.maxSize': (params: any) =>
    `当前文件最大上传大小为${params?.singleFileMaxSize}，请重新上传`,
  'Upload.minSize': (params: any) =>
    `当前文件最小上传大小为${params?.singleFileMinSize}，请重新上传`,
  'Upload.maxNum': (params: any) =>
    `当前文件最大上传个数为${params?.numberLimit}，不允许继续上传`,
  'Upload.format': (params: any) =>
    `文件格式不正确，请上传${params?.acceptType}格式的文件`,
  'Upload.limit': (params: any) =>
    `当前文件最大上传个数为${params?.numberLimit}，请重新选择`,
  ...
  'Transfer.min': (params: any) =>
    `${params.name}最少需要选${params.min}条数据`,
  'Transfer.max': (params: any) =>
    `${params?.name}最多可选${params?.max}条数据`,
  'Table.filter.empty': '空白',
  'Table.expand.component': '已关联业务组件：',
  'Table.data.typeError': (params: any) =>
    `表格数据类型错误，当前类型为${params.type}，请检查配置`,
  'SuperSelect.load': '仅加载50条数据，更多数据请搜索',
  'DatePicker.afterToday': (params: any) =>
    `${params.name}必须在今天之后（含今天）`,
  'DatePicker.beforeToday': (params: any) =>
    `${params.name}必须在今天之前（含今天）`,
  'DatePicker.period': (params: any) =>
    `${params.name}必须在${params.startTime}到${params.endTime}的范围内`,
  'DatePicker.after': (params: any) =>
    `${params.name}不能小于${params.startTime}`,
  'DatePicker.before': (params: any) =>
    `${params.name}不能大于${params.endTime}`,
...
export const lang = (t: string, tip?: unknown) => {
  // @ts-ignore
  const l =
    typeof langOptions[t] === 'function' ? langOptions[t](tip) : langOptions[t];
  return l;
};
