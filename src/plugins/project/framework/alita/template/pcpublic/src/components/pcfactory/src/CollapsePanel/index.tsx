...
const CollapsePanel = LingxiForwardRef<any, MyCollapsePanelProps>(
  (props, ref) => {
    const {
      visible = true,
      children,
    ...
    const { currentIndex, props: currentProps } =
      childRenderContext?.getCurrentData?.(props.$$componentItem.uid) || {
        currentIndex: 0,
        props: {},
      };
    ...
    const renderHeader = () => {
      ...
      const panelHeader = titleColor ? (
        <div style={{ color: titleColor }}>{header}</div>
      ) : (
        header
      );
        ...
        switch (cardIconType) {
          ...
          case 'none':
            return (
              <div
                style={headStyle}
                className="ued-collapse-panel-header-default"
              >
                {panelHeader}
              </div>
            );
    ...
    renderRef.current = visible ? (
      ...
      >
        <div
          ...
          className="ued-collapse-pane-content"
        >
          {children}
        </div>
      </Panel>
