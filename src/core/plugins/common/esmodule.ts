...
export function groupDepsByPack(
  ...
  deps.forEach((dep) => {
    if (dep.dependencyType === DependencyType.Internal) {
      addDep(
        `${(dep as IInternalDependency).moduleName}${
          dep.main ? `/${dep.main}` : ''
        }`,
        dep,
      );
      ...
      addDep(
        `${(dep as IExternalDependency).package}${
          depMain ? `/${depMain}` : ''
        }`,
        dep,
      );
...
function getExportNameOfDep(dep: IDependency): string {
  if (dep.destructuring) {
    return (
      dep.exportName ||
      dep.type ||
      throwNewError('destructuring dependency must have exportName or type')
    );
  }
  ...
  if (!dep.subName) {
    return (
      dep.type ||
      dep.exportName ||
      throwNewError('dependency item must have type or exportName')
    );
  }
...
function buildPackageImport(
  ...
  const depsInfo: IDependencyItem[] = deps.map((dep) => {
    const info: IDependencyItem = {
      ...
      nodeIdentifier: dep.type || undefined,
      source: dep,
    };
  ...
  const nodeIdentifiers = depsInfo
    .map((info) => info.nodeIdentifier)
    .filter(Boolean);
  const conflictInfos = flatMap(Object.keys(exportItems), (exportName) => {
    ...
    const usedNames = [
      ...
      ...(exportItem.needOriginExport || exportItem.aliasNames.length <= 0
        ? [exportName]
        : []),
    ];
    const conflictNames = usedNames.filter(
      (n) => nodeIdentifiers.indexOf(n) >= 0,
    );
    if (conflictNames.length > 0) {
      return [
        ...(conflictNames.indexOf(exportName) >= 0
          ? [[exportName, true, exportItem]]
          : []),
        ...conflictNames
          .filter((n) => n !== exportName)
          .map((n) => [n, false, exportItem]),
      ];
    }
    return [];
  });

  const conflictExports = conflictInfos
    .filter((c) => c[1])
    .map((c) => c[0] as string);
  const conflictAlias = conflictInfos
    .filter((c) => !c[1])
    .map((c) => c[0] as string);
  ...
  depsInfo.forEach((info) => {
    if (info.aliasName && conflictAlias.indexOf(info.aliasName) >= 0) {
      ...
      if (!solution) {
        ...
        const conflictItem = (conflictInfos.find(
          (c) => c[0] === info.aliasName,
        ) || [])[2] as IExportItem;
        conflictItem.aliasNames = conflictItem.aliasNames.filter(
          (a) => a !== info.aliasName,
        );
  ...
  if (useAliasName) {
    Object.keys(exportItems).forEach((exportName) => {
      ...
      if (aliasList.length > 0) {
        const srcName = exportItems[exportName].needOriginExport
          ? exportName
          : aliasList[0];
  ...
  depsInfo.forEach((info) => {
    ...
    if (info.nodeIdentifier) {
      ...
      chunks.push({
        ...
        content: useAliasName
          ? `const ${info.nodeIdentifier} = ${ownerName}.${info.subName};`
          : '',
        linkAfter: [
      ...
      if (
        info.isDefault &&
        defaultExportNames.find((n) => n === info.aliasName)
      ) {
  ...
  if (otherExports.length > 0) {
    const items = otherExports.map((item) => {
      return !useAliasName ||
        item.needOriginExport ||
        item.aliasNames.length <= 0
        ? item.exportName
  ...
  const getInternalDependencyModuleId = () =>
    `@/${(deps[0] as IInternalDependency).type}/${pkg}`;
...
export interface PluginConfig {
  ...
  useAliasName?: boolean; // 是否使用 type 重命名组件 identifier
...
const pluginFactory: BuilderComponentPluginFactory<PluginConfig> = (
  ...
  const plugin: BuilderComponentPlugin = async (pre: ICodeStruct) => {
    ...
    if (ir && ir.deps && ir.deps.length > 0) {
      // deps push 的时候可能是数组或者单个的，为了写法方便
